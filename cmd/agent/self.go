...
func (monitor SelfMonitor) Collect(wg *sync.WaitGroup) {
	...
	log.Println("Начинается сбор данных...")
	defer wg.Done()
	for {
		runtime.ReadMemStats(&memStats)
		log.Println("Данные собраны в MemStats")
		...
		log.Println("Данные собраны в SelfMonitor")
...
func (monitor SelfMonitor) Send(client *http.Client, wg *sync.WaitGroup) {
	defer wg.Done()
	log.Println("Отпарвка метрик на сервер...")

	buildUrl := func(name string, val any) string {
		typ := "gauge"
		switch val.(type) {
		case int64:
			typ = "counter"
